...
def convert_to_llava(args):
    ...
    problems = pd.read_csv(os.path.join(base_dir, f"GQA_Q2Q_{split}.csv"), dtype={'entity_id': str, 'q_id': str, 'image_id': str})
    ...
    if args.image_num == 0:
        args.image_num = len(split_problems)
    ...
    if split == "train":
        ...
        if len(target_format) < len(split_problems) or args.add_sample is True:
