...
def QA(image_id, ambiguous_question, ambiguous_entity, entity_id, ambiguous_question_answer):
    ...
    text1 = f"""USER: {ambiguous_question} Answer in a short answer.
ASSISTANT:"""
    prompt = text1
    ...
    try:
        ...
        inputs = processor(images=image, text=text1, return_tensors="pt").to(device, torch.float16)

        generated_ids = model.generate(**inputs, min_length = 1, max_new_tokens=256, do_sample=False)
