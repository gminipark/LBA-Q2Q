...
from lavis.common.vqa_tools.vqa_eval import VQAEval, ParVQAEval
...
class VQATask(BaseTask):
    ...
    def after_evaluation(self, val_result, split_name, **kwargs):
        ...
        if isinstance(val_result[0]['answer'], list):
            metrics = self._par_report_metrics(result_file=result_file, split=split_name)
        else:
    ...
    @dist_utils.main_process
    def _par_report_metrics(self, result_file, split):
        """
        Use official VQA evaluation script to report metrics on paraphrases
        """
        metrics = {}
        ...
        if split in self.ques_files and split in self.anno_files:
            ...
            vqa_scorer = ParVQAEval(vqa, vqa_result, n=2)
            logging.info("Start VQA evaluation.")
            vqa_scorer.evaluate()
...
class GQATask(VQATask):
    def valid_step(self, model, samples):
        try:
            answers, inputs, scores = model.predict_answers(
                samples=samples,
                answer_list=self.answer_list,
                inference_method=self.inference_method,
                num_beams=self.num_beams,
        ...
        if len(inputs) and len(scores):
            for answer, ques_id, inp, score, gt_ans in zip(answers, question_id, inputs, scores, gt_answers):
                pred_qa_pairs.append({"question_id": ques_id, "pred_ans": answer, 'question': inp, "score": score, "gt_ans": gt_ans})
        else:
            for answer, ques_id, gt_ans in zip(answers, question_id, gt_answers):
                pred_qa_pairs.append({"question_id": ques_id, "pred_ans": answer, 'question': '', "score": 0.0, "gt_ans": gt_ans})
    ...
    def after_evaluation(self, val_result, split_name, **kwargs):
        ...
        if isinstance(val_result[0]['pred_ans'], list):
            metrics = self._par_report_metrics(result_file=result_file, split=split_name)
        else:
            metrics = self._report_metrics(result_file=result_file, split=split_name)
    ...
    def _par_report_metrics(self, result_file, split):
        ...
        for res in results:
            ...
            for pred in preds:
                ...
                vqa_acc = 1 if pred == gt_ans else 0
                vqa_accs.append(min(1.0, vqa_acc))

            acc.append(max(vqa_accs))
