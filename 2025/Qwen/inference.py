...
The task is to use the provided image, and generate a new yes/no question asking about the {ambiguous_entity} referred to the original question.
...
ASSISTANT: """
            ...
            context += f'''Clarifying Questions and Answers:
USER : {generated_question[0]}\n'''

        else : 
            secondary_prompt = f"""You are given an image and an original question that contains an ambiguous entity.
A previous disambiguation question and its answer have already been generated, but they are still insufficient to identify which '{ambiguous_entity}' the original question refers to.

Step 1: Analyze the limitation.
- Consider the previous question-answer pair and reflect on why this information is not sufficient.
- What ambiguity still remains?What additional information is missing that could help distinguish between multiple '{ambiguous_entity}' instances in the image?

Step 2: Generate a new yes/no question.
- Based on your analysis, generate a new question that directly addresses the missing information.
- You must not repeat or rephrase the previous question.
- Instead, use a new attribute, relationship, action, or spatial cue to help resolve the remaining ambiguity.
- If your new question is sufficiently disambiguating, answer it (without saying it explicitly). If not, try again with a better question.
...
When answering questions about direction (such as left or right), always judge direction based on the image itself — the viewer’s perspective — not the orientation or facing direction of the object.
...
USER: {ambiguous_question} Answer only in one or two words.
ASSISTANT:"""
...
def generate_output(image_id, ambiguous_question, ambiguous_entity, additional_question, entity_id):
    ...
    messages = [
        {
            ...
            "content": [
                ...
                {"type": "text", "text": additional_question},
            ],
        }
    ]
    ...
    generated_ids = model.generate(**inputs, min_length = 1, max_new_tokens=256, do_sample=False, temperature=0)
    ...
    return generated_text[0], additional_question
