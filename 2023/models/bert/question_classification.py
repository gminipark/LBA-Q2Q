...
class CustomTrainer(Trainer):
    ...
    def __init__(self, weight=[1.0, 1.0], *args, **kwargs):
    ...
    def compute_loss(self, model, inputs, return_outputs=False):
        ...
        loss_fct = nn.CrossEntropyLoss(weight=torch.tensor(self.weight))
        loss = loss_fct(logits.view(-1, self.model.config.num_labels).cpu(), labels.view(-1).cpu())
...
class ModelArguments:
    ...
    use_loss_weight: bool = field(
        default=False,
        metadata={
            'help': "Whether use weight option in cross-entropy loss"
        }
    )
...
def get_label_weight(raw_dataset, split='train'):
     
    if isinstance(raw_dataset[split]["label"][0], list):
        labels = [label for sample in raw_dataset[split]["label"] for label in sample]
        label_nums = Counter(labels)
    else:
        label_nums = Counter(raw_dataset[split]["label"])
...
def main():
    ...
    if is_regression:
        ...
        label_nums = get_label_weight(raw_datasets, split="train")
    ...
    def preprocess_function(examples):
        ...
        examples['sentence'] = examples['ambiguous_question']
        for idx, item in enumerate(examples['ambiguous_question']):
            examples['sentence'][idx] = "ambiguous question: " + item +" ambigous entity: " + examples["ambiguous_entity"][idx] + " intermediate question: " + examples["intermediate_question"][idx] # + " intermediate answer: " + sample["intermediate_answer"][idx]
            examples['sentence'][idx]  = examples['sentence'][idx]  + " Is the intermediate question effective to clarify the ambiguous entity in the ambiguous question?"
    ...
    if model_args.use_loss_weight:
        weight = [1.0 for idx in range(len(label_nums))]
